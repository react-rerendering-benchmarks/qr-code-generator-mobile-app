...
export default memo(function App() {
  ...
  useEffect(() => {
    (async () => {
      hasPermissions.current = (await MediaLibrary.requestPermissionsAsync()).granted;
    })();
  }, []);
  const saveQRCode = () => {
    if (!hasPermissions.current || !QRref.current) return;
    QRref.current.toDataURL(async data => {
      const QRCodeImg = FileSystem.documentDirectory + "QRCode.png";
      await FileSystem.writeAsStringAsync(QRCodeImg, data, {
        encoding: FileSystem.EncodingType.Base64
      });
      MediaLibrary.saveToLibraryAsync(QRCodeImg).then(() => Toast.show("QR Code saved to gallery", Toast.durations.LONG)).catch(console.error);
    });
  };
  return <View style={styles.container}>      
      ...
      <View style={styles.logoHolder}>
        <Pressable onPress={() => {
        Linking.openURL("https://github.com/whybe7");
      }}>
